...
const fontGenerator = require('./index');
...
function iconFontCreator() {
  ...
  const options = removeUndefinedKeys({
    ...
    styledComponent: args.styledcomponent,
  });
...
function showHelp() {
  console.log(
    'Usage   :'.bold + ' icon-font-creator [ svg-icons-glob ] -o [ output-dir ] [ options ]\n' +
    'Example :'.bold + ' icon-font-creator src/*.svg -o dist\n\n' +

    'Options:\n'.bold +
    '  -o, --out        '.bold + 'Output icon font set files to <out> directory\n'.italic +
    '  -n, --name       '.bold + 'Name to use for generated fonts and files (Default: icon-font)\n' +
    ...
    '  --styledcomponent'.bold + 'Generate styled component in ReactJS'
  )
}
