...
async function generateStyledComponent(options) {
  if (options.styledComponent) {
    ...
    fs.writeFileSync(styledComponentPath, template({ data: 1 }));
  }
}
...
async function generateFont(options) {
  ...
  await generateStyledComponent(options);
...
function getGeneratorConfig(options) {
  ...
  const config = {
    ...
    rename: (name) => {
      let fileName = path.basename(name).replace(/_/g, '-').replace(/ /g, '-');
      let iconName = fileName.substr(0, fileName.lastIndexOf('.'));
      return String(iconName).toLocaleLowerCase();
    },
  }
